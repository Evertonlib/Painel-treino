...
function CartaoTreino({ rotulo, registro }) {
  ...
  return (
    <div className={`relative overflow-hidden rounded-2xl border bg-white p-4 shadow-sm ${estilo.destaque}`}>
      ...
      <div className="mt-3 flex items-center gap-2.5">
        ...
        <div className={`text-lg font-bold ${off ? 'text-slate-500' : 'text-slate-800'}`}>
          {off ? 'Dia de descanso' : registro.tipo}
        </div>
      </div>

      <p className="mt-3 text-sm leading-relaxed text-slate-700">{registro.treino}</p>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        <span className="inline-flex items-center gap-1 rounded-lg bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-600 ring-1 ring-slate-200">
          RPE {registro.rpe}
        </span>
      </div>

      {registro.forca && (
        <div className="mt-3 rounded-lg border border-violet-100 bg-violet-50/60 px-3 py-2 text-xs text-violet-800">
